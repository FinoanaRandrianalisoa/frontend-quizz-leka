...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    ...
    if (!selectedThemeId || !orderedThemes.some((theme) => Number(theme.id) === selectedThemeId)) {
  ...
  const handleRoleChange = async (target: Utilisateur, nextRole: "ADMIN" | "JOUEUR") => {
    if (!isAdmin || target.id === user?.id) return;
    setUpdatingId(target.id);
    try {
  ...
  const loadQuestionForEdit = (question: Question) => {
    ...
    setDraft({ id: Number(question.id), texte: question.texte, answers: filled });
  };
      ...
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-12">
        <Card className="border-dashed border-[#D9D9D9] bg-white/80">
          <CardContent className="p-8 text-center">
            ...
            <h1 className="text-2xl font-bold text-[#2D3142] mb-2">Accès réservé</h1>
            <p className="text-[#A0A0A0]">Seuls les administrateurs peuvent consulter le tableau de bord.</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  const summary = [
    { label: "Joueurs", value: stats.data?.joueurs ?? 0, icon: Users, tint: "bg-[#dcfce7] text-[#15803d]" },
    { label: "Questions", value: stats.data?.questions ?? 0, icon: Sparkles, tint: "bg-[#ecfdf5] text-[#166534]" },
    { label: "En ligne", value: stats.data?.joueursEnLigne ?? 0, icon: Activity, tint: "bg-[#f0fdf4] text-[#16a34a]" },
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 pb-20 md:pb-8">
      <div className="mb-6 overflow-hidden rounded-2xl bg-gradient-to-r from-[#16a34a] to-[#22c55e] p-4 text-white shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-[0.22em] text-white/70 font-semibold">Administration</p>
            <h1 className="text-2xl md:text-3xl font-black">Dashboard administrateur</h1>
          </div>
          <div className="flex items-center gap-2">
            <Badge variant="category" className="border-white/20 bg-white/15 text-white">Système actif</Badge>
            <Button variant="secondary" size="sm" onClick={() => void refreshAll()} className="bg-white text-[#166534] hover:bg-[#f0fdf4]">
              Rafraîchir
            </Button>
          </div>
        </div>
          ...
          <Card key={label} className="border-[#E8F5EB] bg-white shadow-sm">
            <CardContent className="p-4 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-2xl font-black text-[#1f2a1f] mt-2">{value}</p>
              </div>
          ...
          <CardContent className="p-0">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-left">
                <thead className="bg-[#f3faf4] text-[#64748b] text-xs uppercase tracking-wide">
                  <tr>
                ...
                <tbody>
                  {users.loading && (
                    <tr>
                      <td colSpan={4} className="px-4 py-8 text-center text-[#A0A0A0]">Chargement des utilisateurs…</td>
                    </tr>
                  )}

                  {!users.loading && filteredUsers.length === 0 && (
                    <tr>
                      <td colSpan={4} className="px-4 py-8 text-center text-[#A0A0A0]">Aucun utilisateur trouvé.</td>
                    </tr>
                  )}
                  ...
                  {!users.loading && filteredUsers.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className="border-t border-[#edf6ef] hover:bg-[#f9fdf9]">
                        ...
                        <td className="px-4 py-3 text-sm text-[#1f2a1f]">{item.email}</td>
                        <td className="px-4 py-3">
                          <Badge variant={isAdminUser ? "playing" : "default"} className="text-[10px]">
                            {item.role}
                          </Badge>
                        </td>
                        <td className="px-4 py-3 text-right">
                          <div className="flex justify-end gap-2">
                            <Button
                              size="sm"
                              variant={isAdminUser ? "secondary" : "success"}
            ...
            <CardContent className="p-4">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between items-center border-b border-[#edf6ef] pb-2">
                  ...
                  <span className="font-bold text-[#1f2a1f]">{filteredUsers.filter((u) => u.role === "ADMIN").length}</span>
                </div>
                <div className="flex justify-between items-center border-b border-[#edf6ef] pb-2">
                  <span className="text-[#64748b]">Thèmes</span>
                  <span className="font-bold text-[#1f2a1f]">{themes.data?.length ?? 0}</span>
                </div>
                <div className="flex justify-between items-center border-b border-[#edf6ef] pb-2">
          ...
          <Card className="border-[#E8F5EB] bg-white shadow-sm">
            <CardContent className="p-4">
              ...
              <div className="space-y-3 text-sm text-[#1f2a1f]">
                ...
                <div className="flex items-center justify-between rounded-xl bg-[#f9fdf9] p-3">
                  <span>Questions</span>
                  <span className="font-bold">{stats.data?.questions ?? 0}</span>
                </div>
                <div className="flex items-center justify-between rounded-xl bg-[#f9fdf9] p-3">
                  <span>App live</span>
        ...
        <CardContent className="p-0">
          <div className="flex flex-col gap-4 p-4 border-b border-[#F0F0F0] md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-bold text-[#2D3142]">Gestion des questions</h2>
            </div>
            <div className="flex flex-wrap gap-2">
              {orderedThemes.map((theme) => (
                <Button
                  key={theme.id}
                  size="sm"
                  variant={selectedThemeId === Number(theme.id) ? "default" : "secondary"}
          ...
          <div className="grid grid-cols-1 gap-6 p-4 xl:grid-cols-[1.1fr_0.9fr]">
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-3">
                <h3 className="font-semibold text-[#2D3142]">Questions du thème</h3>
                <Button size="sm" variant="success" onClick={resetDraft} className="gap-2">
                  <Plus size={14} />
                  Nouvelle question
                </Button>
              </div>
              ...
              {questionsLoading ? (
                <div className="text-sm text-[#A0A0A0]">Chargement des questions…</div>
              ) : questions.length === 0 ? (
                ...
                <div className="space-y-3">
                  {questions.map((question) => (
                    <div key={question.id} className="rounded-xl border border-[#E8E8E8] bg-[#F9F9F9] p-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1">
                          <p className="font-semibold text-[#2D3142]">{question.texte}</p>
                          <div className="mt-2 flex flex-wrap gap-2">
                            {question.choix.map((choice) => (
                              <Badge key={choice.id} variant={choice.estCorrecte ? "won" : "default"} className="text-[10px]">
                                {choice.texte}
                        ...
                        <div className="flex gap-2">
                          <Button size="sm" variant="secondary" onClick={() => loadQuestionForEdit(question)} className="gap-2">
                            <Pencil size={14} />
                            Modifier
                          </Button>
                          <Button size="sm" variant="destructive" onClick={() => handleDeleteQuestion(Number(question.id))} className="gap-2">
                            <Trash2 size={14} />
                            Supprimer
                          </Button>
                        </div>
            ...
            <div className="rounded-xl border border-[#E8E8E8] bg-[#FAFAFA] p-4">
              ...
              <div className="space-y-4">
                <Input
                  label="Question"
                  value={draft.texte}
                  onChange={(e) => setDraft((current) => ({ ...current, texte: e.target.value }))}
                  placeholder="Saisissez la question"
                />

                <div className="space-y-3">
                  {draft.answers.map((answer, index) => (
                    <div key={index} className="rounded-lg border border-[#E8E8E8] bg-white p-2">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-semibold text-[#A0A0A0]">{index + 1}</span>
                        <Input
                          value={answer.texte}
                        ...
                        >
                          {answer.estCorrecte ? <Check size={14} /> : "Bonne réponse"}
                        </Button>
                      </div>
                    </div>
                  ))}
                ...
                <div className="flex justify-end gap-2 pt-2">
                  ...
                  <Button variant="default" onClick={() => void handleSaveQuestion()} loading={savingQuestion}>
