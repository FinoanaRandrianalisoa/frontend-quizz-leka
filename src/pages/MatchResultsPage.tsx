      ...
      <div className="w-full max-w-xl">
        <div
          className={`rounded-2xl p-8 text-center text-white mb-6 ${
            won
              ? "bg-gradient-to-br from-[#004E89] to-[#002952]"
              : "bg-gradient-to-br from-[#D62828] to-[#8B0000]"
          }`}
        >
          ...
          <div className="flex items-center justify-center gap-6">
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-4xl font-black tabular-nums text-[#FFD700]">
                {m.scoreHote}
              </span>
            </div>
            ...
            <div className="flex flex-col items-center gap-2">
              <Avatar size="lg">
                <AvatarFallback>
                  {initial(m.joueurInvite?.pseudo)}
                </AvatarFallback>
              </Avatar>
              <span className="font-semibold text-sm">
                {m.joueurInvite ? <UserName user={m.joueurInvite} /> : "—"}
              </span>
              <span className="text-4xl font-black tabular-nums text-white/60">
                {m.scoreInvite}
              </span>
            </div>
          </div>
          ...
          <Card className="mb-4 border-[#06A77D]/30 bg-[#06A77D]/5">
            <CardContent className="p-4 flex flex-col gap-3">
              ...
              <p className="text-sm text-[#2D3142]">
                Vous avez atteint le score cible de {m.scoreCible} points et
                gagné la partie.
              </p>
              {!published && (
                <Button
              ...
              {published && (
                <Badge variant="won" className="self-start">
                  Victoire publiée
                </Badge>
              )}
            </CardContent>
        ...
        <Card className="mb-4">
          <CardContent className="p-4 flex flex-col gap-3">
            ...
            {[
              {
                label: "Score hôte",
                value: String(m.scoreHote),
                icon: Check,
                color: "#06A77D",
              },
              {
                label: "Score invité",
                value: String(m.scoreInvite),
                icon: X,
                color: "#D62828",
              },
              {
                label: "Objectif",
                value: String(m.scoreCible),
                icon: Star,
                color: "#FFD700",
              },
              {
                label: "Tours",
                value: String(m.tourActuel),
                icon: Clock,
                color: "#FF6B35",
              },
        ...
        {opp && (
          <Card className="mb-6">
            <CardContent className="p-4 flex items-center gap-3">
              <Avatar size="md">
                <AvatarFallback>{initial(opp.pseudo)}</AvatarFallback>
              </Avatar>
