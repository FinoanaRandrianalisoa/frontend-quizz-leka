...
type NotificationItem = {
  ...
  creeLe?: string;
  expediteur?: { pseudo?: string | null; photoProfil?: string | null; firstName?: string | null; villeOrigine?: string | null } | null;
};

type SenderUser = { pseudo?: string | null; photoProfil?: string | null; firstName?: string | null; villeOrigine?: string | null } | null;
...
const isRabbitInvitationNotification = (item: Pick<NotificationItem, "type" | "titre" | "message">) => {
  if (!item) return false;
  const titre = (item.titre ?? "").trim().toLowerCase();
  const haystack = `${item.titre ?? ""} ${item.message ?? ""}`.toLowerCase();
  return titre === "invitation course lapin" || (item.type === "defi_recu" && haystack.includes("course lapin"));
};
...
export default function NotificationsPage({ onNavigate }: { onNavigate?: (p: string, id?: number | null) => void }) {
  ...
  const notificationsQuery = useAsync<NotificationItem[]>(() => api.mesNotifications().then((d) => d.mesNotifications ?? []), []);
  const demandes = useAsync(() => api.demandesAmisRecues().then((d) => d.demandesAmisRecues), []);
  ...
  useEffect(() => {
    ...
    window.addEventListener(refreshNotificationEvent, handleRefresh as EventListener);
    return () => window.removeEventListener(refreshNotificationEvent, handleRefresh as EventListener);
  ...
  const notifications = useMemo(() => {
    ...
    for (const item of notificationsData) {
      ...
      const isChallenge = item.type === "defi_recu" || item.type === "defi_accepte" || isRabbitInvite || isRabbitReadyNotification(item);
      items.push({
        id: String(item.id),
  ...
  const handleAccept = async (notificationId: string, requestId?: string) => {
    const id = requestId ? Number(requestId.replace("friend-", "")) : Number(notificationId);
    if (requestId) {
  ...
  const refuseRabbitInvitation = async (matchId: number, notificationId: string) => {
    try {
  ...
  const acceptRabbitInvitation = async (matchId: number, notificationId?: string) => {
    if (notificationId) {
      await api.marquerNotificationLue(Number(notificationId)).catch(() => undefined);
    }
    try {
    ...
    <div className="max-w-2xl mx-auto px-4 md:px-6 py-6 pb-20 md:pb-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          {unread > 0 && <p className="text-sm text-[#64748b]">{unread} non lues</p>}
        </div>
        <Button variant="ghost" size="sm" className="gap-1.5 text-[#64748b]" onClick={markAllRead}>
            ...
            return (
              <div
                ...
                className={`flex items-start gap-3 px-4 py-4 transition-colors cursor-pointer
                  ${n.read ? "hover:bg-[#f3faf4]" : "bg-[#16a34a]/5 hover:bg-[#16a34a]/10 border-l-2 border-[#16a34a]"}
                  ${i < notifications.length - 1 ? "border-b border-[#edf6ef]" : ""}`}
                onClick={() => { void handleNotificationClick(n); }}
              >
                {n.user ? (
                  <Avatar size="md" className="shrink-0 shadow-sm">
                    {n.user.photoProfil
                      ? <AvatarImage src={n.user.photoProfil} alt={n.user.pseudo ?? ""} />
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-semibold text-[#1f2a1f]">{n.user ? <UserName user={n.user} /> : n.title}</p>
                    {!n.read && <span className="w-2 h-2 rounded-full bg-[#16a34a] shrink-0" />}
                  </div>
                  {n.user && <p className="text-xs font-medium text-[#16a34a] mt-0.5">{n.title}</p>}
                ...
                {n.type === "challenge" && !n.read && (() => {
                  ...
                  if (rabbitInvite && rabbitMatchId) {
                    return (
                      <div className="flex shrink-0 flex-col gap-1">
                        <Button size="sm" onClick={(e) => { e.stopPropagation(); void acceptRabbitInvitation(Number(rabbitMatchId), n.id); }}>
                          Accepter
                        </Button>
                  ...
                  return (
                    <Button size="sm" className="shrink-0" onClick={async (e) => { e.stopPropagation(); await startChallengeFromNotification(); }}>
                      Voir le défi
                    </Button>
                  );
                })()}
