...
export default function SpectatorPage({
  ...
  const paris = useAsync(
    () =>
      selectedMatch
        ? api.parisMatch(Number(selectedMatch.id))
        : Promise.resolve({ parisMatch: [] }),
    [selectedMatch?.id],
  )
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-[#F9F9F9]">
      <div className="bg-[#2D3142] text-white px-4 py-2 flex items-center gap-3 shrink-0">
        ...
        <span className="text-sm font-medium">
          {selectedMatch
            ? `${displayName(selectedMatch.joueurHote)} vs ${
                selectedMatch.joueurInvite
                  ? displayName(selectedMatch.joueurInvite)
                  : "…"
              }`
      ...
      <div className="flex flex-1 overflow-hidden">
                ...
                <button
                  ...
                  className={`w-full text-left rounded-2xl border p-3 transition-all ${
                    active
                      ...
                      : "border-[#E8E8E8] bg-white hover:border-[#FF6B35]/40"
                  }`}
                >
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <div
                        ...
                        style={{ background: themeColor(m.theme.nom) }}
                      >
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 flex gap-4">
          {!selectedMatch && !live.loading && (
            <div className="h-full flex items-center justify-center text-sm text-[#A0A0A0]">
              Choisissez un match pour assister à la partie.
            </div>
          )}
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-[#F5F5F5]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-[#A0A0A0]">
                      {selectedMatch.theme.nom} · objectif{" "}
                      {selectedMatch.scoreCible}
                    </span>
                  </div>
                  <div className="flex items-center gap-1 text-[#A0A0A0] text-xs">
                    <Users size={12} />{" "}
                    {selectedMatch.joueurInvite ? "2 joueurs" : "1 joueur"}
                  </div>
                </div>
                ...
                <div className="p-5">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
                    <div className="flex items-center gap-3">
                      <Avatar
                        size="lg"
                        online={selectedMatch.joueurHote.enLigne}
                      >
                        <AvatarFallback>
                          {initial(selectedMatch.joueurHote.pseudo)}
                        </AvatarFallback>
                      </Avatar>
                      <div>
                        <p className="text-xs text-[#A0A0A0]">Joueur 1</p>
                        <p className="font-bold text-[#2D3142]">
                          <UserName user={selectedMatch.joueurHote} />
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        ...
                        <p className="font-bold text-[#2D3142]">
                          {selectedMatch.joueurInvite ? (
                            <UserName user={selectedMatch.joueurInvite} />
                          ) : (
                            "En attente"
                          )}
                        </p>
                      ...
                      <Avatar
                        size="lg"
                        online={selectedMatch.joueurInvite?.enLigne}
                      >
                        <AvatarFallback>
                          {initial(selectedMatch.joueurInvite?.pseudo)}
                        </AvatarFallback>
                      </Avatar>
                    </div>
                  </div>
                  ...
                  {currentQuestion ? (
                    <div className="space-y-4">
                      <div className="rounded-2xl bg-[#F5F5F5] p-4 border border-[#E8E8E8]">
                        <div className="flex items-center justify-between mb-3">
                          <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#A0A0A0]">
                            Question {selectedMatch.tourEnCours?.numero ?? 1}
                          </span>
                          <span className="text-xs text-[#A0A0A0]">
                            {currentQuestion.theme.nom}
                          </span>
                        </div>
                        <p className="text-lg font-bold text-[#2D3142] leading-relaxed">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {currentQuestion.choix.map((option, index) => (
                          <div
                            key={option.id}
                            className="rounded-2xl border border-[#D9D9D9] bg-white p-3 text-sm text-[#2D3142] font-medium"
                          >
                            <span className="mr-2 text-[#FF6B35] font-bold">
                              {["A", "B", "C", "D"][index]}.
                            </span>
                            {option.texte}
                          </div>
              ...
              <div className="w-[320px] bg-white border border-[#E8E8E8] rounded-3xl overflow-hidden">
                ...
                <div className="p-4 space-y-4">
                  {peutParier ? (
                    <div className="space-y-3">
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <button
                            onClick={() =>
                              setSelectedJoueurId(
                                Number(selectedMatch.joueurHote.id),
                              )
                            }
                          ...
                          {selectedMatch.joueurInvite && (
                            <button
                              onClick={() =>
                                setSelectedJoueurId(
                                  Number(selectedMatch.joueurInvite?.id),
                                )
                              }
                              className={`p-3 rounded-xl border-2 text-center transition-all ${
                                selectedJoueurId ===
                      ...
                      <Button
                        ...
                        disabled={
                          !selectedJoueurId || !montantMise || bettingLoading
                        }
                      ...
                      <p className="text-[10px] text-[#A0A0A0] text-center">
                        Les paris sont fermés dès qu'un joueur atteint 1/3 du
                        score cible.
                      </p>
                    </div>
                  ) : (
                  ...
                  <div className="border-t border-[#F5F5F5] pt-4">
                    <p className="text-xs font-semibold text-[#A0A0A0] mb-3">
                      Paris en cours ({parisData.length})
                    </p>
                        ...
                        parisData.map((pari) => (
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <p className="text-xs font-medium text-[#2D3142]">
                                  <UserName user={pari.joueurPari} />
                                </p>
                                <p className="text-[10px] text-[#A0A0A0]">
                                  {pari.montant} Ar
                                </p>
          ...
          <div className="w-[360px] hidden md:block">
            <ChatPanel
              spectators={matches.length}
              className="h-full"
              inputId="spectator-chat-input"
