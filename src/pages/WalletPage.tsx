...
export default function WalletPage() {
  usePageTitle("Wallet")
  const portefeuille = useAsync(
    () => api.monPortefeuille().then((d) => d.monPortefeuille),
    [],
  )
  const historique = useAsync(
    ...
    <div className="max-w-2xl mx-auto px-4 md:px-6 py-6 pb-20 md:pb-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-[#A0A0A0]">
          Vos points et récompenses en temps réel
        </p>
      </div>
      ...
      <div className="rounded-2xl bg-gradient-to-br from-[#004E89] to-[#002952] text-white p-6 mb-4">
        <div className="flex items-center gap-2 mb-2">
          {pf?.modeDemo && (
            <Badge className="bg-[#FF6B35]/20 text-[#FFD9C4] border border-[#FF6B35]/40">
              DÉMO
            </Badge>
          )}
          {requirePin && (
        ...
        <p className="text-5xl font-extrabold mb-1 tabular-nums">
          {formatPoints(total)}{" "}
          <span className="text-2xl font-semibold text-white/70">pts</span>
        </p>
        <p className="text-sm text-white/60">
          Disponible {formatPoints(disponible)} · Misé {formatPoints(bloque)} ·
          Gains {formatPoints(gains)}
        </p>
        ...
        {!requirePin && (
          <p className="text-xs text-white/60 mt-3">
            Définissez votre PIN pour déverrouiller le wallet et recharger.
          </p>
        )}
        {requirePin && !unlocked && (
          <p className="text-xs text-white/60 mt-3">
            Votre wallet est verrouillé — entrez votre PIN ci-dessous.
          </p>
        )}
        {requirePin && unlocked && !rechargeDispo && (
          <p className="text-xs text-white/60 mt-3">
            Recharge démo déjà utilisée aujourd'hui (limite 1×/jour).
          </p>
        )}
      </div>
          ...
          {!requirePin ? (
            <div className="grid gap-3">
              ...
              <Button
                ...
                onClick={() =>
                  void run(
                    () => api.definirPinPortefeuille(pin, pinConfirmation),
            ...
            <div className="flex items-center justify-between">
              <p className="text-sm text-[#06A77D] flex items-center gap-2">
                <Unlock size={16} /> Portefeuille déverrouillé
              </p>
              <Button
                variant="ghost"
                size="sm"
            ...
            <div className="grid gap-3">
              ...
              <Button
                ...
                onClick={() =>
                  void run(
                    () => api.deverrouillerPortefeuille(pin),
          ...
          {requirePin && showChangerPin && (
            <div className="grid gap-3 border-t border-[#F5F5F5] pt-3">
              <Input
                ...
                maxLength={4}
                value={pinActuel}
                onChange={(e) => setPinActuel(e.target.value)}
              />
              <Input
                type="password"
                inputMode="numeric"
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          {
            label: "Disponible",
            value: formatPoints(disponible),
            trend: "OK",
            up: true,
          },
          {
            label: "Misé / bloqué",
            value: formatPoints(bloque),
            trend: "OK",
            up: true,
          },
      ...
      <Tabs defaultValue="all">
              ...
              {allTx.map((t, i) => (
                <div
                  ...
                  className={`flex items-center gap-3 px-4 py-3 ${
                    i < allTx.length - 1 ? "border-b border-[#F5F5F5]" : ""
                  }`}
                >
                  <div
                    className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                      t.credit ? "bg-[#06A77D]/10" : "bg-[#D62828]/10"
                    }`}
                  >
                    <t.icon
                      size={16}
                      className={t.credit ? "text-[#06A77D]" : "text-[#D62828]"}
                    />
                  </div>
                  ...
                  <span
                    className={`text-sm font-bold tabular-nums shrink-0 ${
                      t.credit ? "text-[#06A77D]" : "text-[#D62828]"
                    }`}
                  >
                    {t.credit ? "+" : ""}
                    {formatPoints(t.amount)} pts
                  </span>
                </div>
              ))}
            </CardContent>
        ...
        <TabsContent value="credits" className="mt-4">
          <Card>
            <CardContent className="p-0">
              {allTx.filter((t) => t.credit).length === 0 && (
                <div className="p-4 text-sm text-[#A0A0A0]">
              ...
              {allTx
                ...
                .map((t, i, arr) => (
                  <div
                    key={t.id}
                    className={`flex items-center gap-3 px-4 py-3 ${
                      i < arr.length - 1 ? "border-b border-[#F5F5F5]" : ""
                    }`}
                  >
                    ...
                    <span className="text-sm font-bold text-[#06A77D] tabular-nums">
                      +{formatPoints(t.amount)} pts
                    </span>
                  </div>
                ))}
              ...
              {allTx
                ...
                .map((t, i, arr) => (
                  ...
                  >
                    ...
                    <span className="text-sm font-bold text-[#D62828] tabular-nums">
                      {formatPoints(t.amount)} pts
                    </span>
                  </div>
                ))}
