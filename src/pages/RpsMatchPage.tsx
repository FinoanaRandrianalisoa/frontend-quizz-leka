...
export default function RpsMatchPage({
  onNavigate,
}: {
  ...
  const loadPendingChallenge = async () => {
    ...
    try {
      ...
      const activeMatch = (res.mesMatchsRps ?? []).find(
      ...
      if (
        nextMatch.round === 1 &&
        nextMatch.scoreHote === 0 &&
        nextMatch.scoreInvite === 0
      ) {
        setCountdown(5)
      }
    } catch (err: any) {
  ...
  const pollMatch = async () => {
    ...
    try {
      ...
      const currentMatch = (res.mesMatchsRps ?? []).find(
  ...
  const meIsInvite =
    !!match && String(user?.id) === String(match.joueurInviteId)
  ...
  const statusText = useMemo(() => {
    ...
    if (match.statut === "termine") {
      if (match.scoreHote === match.scoreInvite)
  ...
  const handlePublishVictory = async () => {
    ...
    try {
      ...
      await api.publier(
        `🎉 ${winner} a gagné un duel Pierre-Papier-Ciseaux contre ${loser} ! Score final : ${match.scoreHote} - ${match.scoreInvite}`,
      )
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6">
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex items-center justify-center gap-3">
              <Badge
                variant={match.statut === "termine" ? "won" : "playing"}
                className="text-xs font-bold px-4 py-2 shadow-lg"
              >
                {statusText}
              </Badge>
              {meIsSpectator && (
            ...
            {countdown !== null && (
              <div className="rounded-3xl border-2 border-[#FF6B35] bg-gradient-to-br from-[#FF6B35]/20 to-[#FFD700]/20 backdrop-blur-sm p-8 text-center shadow-2xl">
                <p className="text-sm font-semibold text-[#FF6B35] mb-2 uppercase tracking-wider">
                  Le duel commence dans
                </p>
                <p className="text-7xl font-black text-[#FF6B35] drop-shadow-lg animate-pulse">
                  {countdown}
                </p>
              </div>
            )}
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 shadow-xl">
              ...
              <div className="mt-4 p-4 rounded-2xl bg-white/5 border border-white/10 text-center">
                <p className="text-sm text-white/80">
                  {meIsSpectator
                    ? "👁️ Vous regardez ce duel en tant que spectateur"
                    : meIsHost
                      ? "🎯 Vous êtes l'hôte du défi"
                      : "🎯 Vous êtes l'invité du défi"}
                </p>
              </div>
            </div>
                      ...
                      {rematchAccepted && (
                        <div className="p-4 rounded-2xl bg-[#4CAF50]/20 border border-[#4CAF50]/50 text-center">
                          <p className="text-sm font-semibold text-[#4CAF50]">
                            ✓ Revanche acceptée ! Nouveau duel qui commence...
                          </p>
                        </div>
                      )}
