...
import RabbitLudoBoard, {
  Dice3D,
  FINISH_POS,
  VictoryOverlay,
} from "../components/game/RabbitLudoBoard"
...
function toPlayer(
  ...
  return {
    ...
    color:
      DEFAULT_COLORS[isHost ? 0 : id % DEFAULT_COLORS.length] ??
      DEFAULT_COLORS[0],
...
function playersFromMatch(match: Match, currentUserId?: number): Player[] {
  ...
  if (host?.id)
    list.push(toPlayer(Number(host.id), host.pseudo, true, host.photoProfil))

  // L'invité est prêt s'il a accepté

  if (invite?.id && match.inviteAccepte) {
    list.push(
      toPlayer(Number(invite.id), invite.pseudo, false, invite.photoProfil),
    )
  }
  ...
  if (
    currentUserId &&
    invite &&
    Number(invite.id) === currentUserId &&
    !list.some((p) => Number(p.id) === currentUserId)
  ) {
    list.push(
      toPlayer(Number(invite.id), invite.pseudo, false, invite.photoProfil),
    )
...
export default function RabbitRacePage({
  onNavigate,
  matchId,
}: {
  ...
  const [inviteStatus, setInviteStatus] =
    useState<Record<number, InviteStatus>>({})
  ...
  const [currentMatchId, setCurrentMatchId] = useState<number | null>(
    matchId ?? null,
  )
  ...
  useEffect(() => {
    raceLiveRef.current =
      started || pendingStart || Boolean(winner) || walkingRef.current
  }, [pendingStart, started, winner])

  const walkPlayerTo = useCallback(
    async (playerId: number, targetPos: number) => {
      ...
      for (let next = start + 1; next <= dest; next += 1) {
        setPlayers((prev) =>
          prev.map((p) => (p.id === playerId ? { ...p, pos: next } : p)),
        )
  ...
  const applyMatch = useCallback(
    (rabbitMatch: Match | null | undefined) => {
      ...
      if (invitedUserId) {
        setInviteStatus((prev) => ({
          ...prev,
          [invitedUserId]: accepted ? "accepted" : "pending",
        }))
      }

      if (
        rabbitMatch.statut === "en_cours" &&
        !started &&
        !pendingStart &&
        !winner
      ) {
    ...
    const loadMatch = async () => {
      ...
      try {
        ...
        const isCurrentUserInvite =
          rabbitMatch.joueurInvite &&
          Number(rabbitMatch.joueurInvite.id) === Number(user?.id)

        const currentUserInPlayers = players.some(
          (p) => Number(p.id) === Number(user?.id),
        )

        if (
          !loadedMatchRef.current ||
          (isCurrentUserInvite && !currentUserInPlayers)
        ) {
  ...
  useEffect(() => {
    if (currentMatchId || matchId || !user?.id || creatingMatchRef.current)
      return
    ...
    void api
      ...
      .then((res) => {
        ...
        setPlayers([
          toPlayer(Number(user.id), user.pseudo, true, user.photoProfil),
        ])
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(
      `${wsBase}/ws/match/${String(currentMatchId)}/?token=${encodeURIComponent(token ?? "")}`,
    )
    ...
    const pollingInterval = setInterval(
      async () => {
        ...
        try {
          ...
          console.log(
            "[Polling] inviteAccepte:",
            rabbitMatch.inviteAccepte,
            "players.length:",
            players.length,
            "joueurInvite:",
            rabbitMatch.joueurInvite?.id,
          )
          ...
          if (
            ...
            setPlayers((prev) => {
              if (prev.some((entry) => Number(entry.id) === inviteId))
                return prev

              const host =
                prev.find((entry) => entry.isHost) ??
                toPlayer(
                  Number(user?.id ?? 0),
                  user?.pseudo ?? "Hôte",
                  true,
            ...
            setNotifications((prev) =>
              [
                { id: Date.now(), text: "Tous les joueurs sont prêts." },
                ...prev,
              ].slice(0, 5),
            )
      ...
      try {
        ...
        if (payload.type === "rabbit.invite_sent") {
          ...
          if (inviteId)
            setInviteStatus((prev) => ({
              ...prev,
              [inviteId]: prev[inviteId] ?? "pending",
            }))
        }
        ...
        if (
          payload.type === "rabbit.move" &&
          Number(payload.from_id) !== Number(user?.id)
        ) {
        ...
        if (payload.type === "rabbit.win") {
          ...
          const known = playersRef.current.find(
            (p) => Number(p.id) === winnerId,
          )
        ...
        if (payload.type === "rabbit.rematch_request") {
          ...
          setMoveHint(
            requesterId === Number(user?.id)
              ? "Votre demande de revanche est en attente de validation."
              : "Une revanche a été proposée. Acceptez pour relancer la partie.",
          )
        }
  ...
  useEffect(() => {
    ...
    const nextText = currentPlayer
      ? isMyTurn
        ? `C'est votre tour, ${currentPlayer.name}`
  ...
  const invitePlayer = async (player: Utilisateur) => {
    ...
    if (!currentMatchId) {
      setNotifications((prev) =>
        [
          { id: Date.now(), text: "La partie n'est pas encore créée." },
          ...prev,
        ].slice(0, 5),
      )
    ...
    try {
      ...
      setNotifications((prev) =>
        [
          {
            id: Date.now(),
            text: `Invitation envoyée à ${displayName(player)}`,
          },
          ...prev,
        ].slice(0, 5),
      )
      ...
      setNotifications((prev) =>
        [
          {
            id: Date.now(),
            text: `Impossible d'envoyer l'invitation à ${displayName(player)}`,
          },
          ...prev,
        ].slice(0, 5),
      )
  ...
  const cancelInvitation = async (playerId: number) => {
    ...
    setNotifications((prev) =>
      [{ id: Date.now(), text: "Invitation annulée." }, ...prev].slice(0, 5),
    )
  }
  ...
  const shareVictory = async () => {
    if (
      !winner ||
      Number(winner.id) !== Number(user?.id) ||
      sharingVictory ||
      victoryShared
    )
      return
    ...
    try {
      await api.publier(
        `🏆 Victoire Course des Lapins ! ${winner.name} a atteint la cage ${FINISH_POS} et remporte la course.`,
      )
      ...
      setNotifications((prev) =>
        [
          { id: Date.now(), text: "Impossible de publier la victoire." },
          ...prev,
        ].slice(0, 5),
      )
    } finally {
  ...
  const rollDice = async () => {
    ...
    if (projected > FINISH_POS) {
      setMoveHint(
        `${currentPlayer.name} a fait ${value} : trop loin. Il faut un dé exact pour la cage ${FINISH_POS}.`,
      )
  ...
  const rematchPending = Boolean(
    rematchRequestedBy !== null && rematchRequestedBy !== Number(user?.id),
  )

  const allParticipantsAcceptedRematch =
    players.length > 0 && rematchAcceptedCount >= players.length
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#f2fff3_0%,_#edf7ef_30%,_#eaf2ea_100%)] p-4 md:p-8">
      ...
      <header className="mb-6 flex items-center justify-between gap-3 rounded-[28px] border border-white/60 bg-white/65 p-4 shadow-[0_18px_45px_rgba(15,23,42,0.08)] backdrop-blur-sm">
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-[#122217]">
            Course des Lapins
          </h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            variant="ghost"
            onClick={quitMatch}
            className="rounded-full border border-red-200 bg-red-50 text-red-700 shadow-sm hover:bg-red-100"
          >
            Quitter
          </Button>
          {isHostView && (
            <Button
              ...
              onClick={startRace}
              disabled={!canLaunch}
              className="rounded-full bg-gradient-to-r from-emerald-500 to-lime-500 text-white shadow-[0_12px_24px_rgba(34,197,94,0.35)] hover:brightness-105"
            >
              <Play size={16} />
            </Button>
          )}
            ...
            <CardContent className="space-y-5 rounded-[28px] border border-white/60 bg-white/70 p-5 shadow-[0_25px_60px_rgba(15,23,42,0.08)] backdrop-blur-sm">
              <div className="flex items-center justify-between">
                ...
                <div className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">
                  {creatingMatch
                    ? "Création…"
                    : `Joueurs ${acceptedPlayers.length}/2`}
                </div>
              </div>
              ...
              {isHostView && (
                <div className="space-y-2">
                  ...
                  {onlineUsers.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-[#64748b]">
                    ...
                    onlineUsers.map((player) => {
                      ...
                      return (
                        <div
                          key={player.id}
                          className="flex items-center justify-between rounded-2xl border border-slate-200 bg-gradient-to-r from-slate-50 to-white p-3 shadow-[0_8px_20px_rgba(15,23,42,0.04)]"
                        >
                          <div className="flex items-center gap-2">
                            <UserAvatar
                              user={player}
              ...
              {!isHostView && (
                <div className="rounded-2xl border border-dashed border-emerald-200 bg-gradient-to-r from-emerald-50 to-lime-50 p-4 text-sm text-emerald-800 shadow-[inset_0_1px_0_rgba(255,255,255,0.85)]">
                  Vous avez rejoint le lobby. Attendez que l’hôte lance la
                  partie une fois que tout le monde est prêt.
                </div>
              )}
                ...
                {acceptedPlayers.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {acceptedPlayers.map((player: Player) => (
                      <div
                        key={player.id}
                        className="flex items-center justify-between rounded-2xl bg-gradient-to-r from-white to-slate-100 px-3 py-2 shadow-[0_10px_18px_rgba(15,23,42,0.04)]"
                      >
                        <div className="flex items-center gap-2">
                          <div
                            className="h-3.5 w-3.5 rounded-full shadow-[0_0_0_3px_rgba(255,255,255,0.9)]"
                        ...
                        {player.isHost ? (
                          <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.15em] text-emerald-700">
                            Hôte
                          </span>
                        ) : (
                          <CheckCircle2
              ...
              {isHostView && (
                <div className="pt-1">
                  <Button
                    onClick={startRace}
                    disabled={!canLaunch}
                    className="w-full rounded-2xl bg-gradient-to-r from-emerald-600 via-lime-500 to-emerald-500 text-white shadow-[0_18px_30px_rgba(34,197,94,0.35)]"
                  >
                    {allPlayersReady
                      ? "Lancer (compte à rebours 5 s)"
              ...
              {notifications.length > 0 && (
                <div className="rounded-[22px] border border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50 p-3 shadow-[0_10px_25px_rgba(251,146,60,0.08)]">
                  <div className="mb-2 text-[10px] font-bold uppercase tracking-[0.2em] text-amber-700">
                    Notifications
                  </div>
                  <div className="space-y-1">
                    {notifications.map((n) => (
                      <div key={n.id} className="text-xs text-amber-800">
                        • {n.text}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-col gap-5 lg:flex-row lg:items-start">
                    ...
                    <aside className="w-full lg:w-72">
                      <div className="mb-4 rounded-[22px] border border-white/10 bg-white/12 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.1)]">
                        <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-100/80">
                          Joueur actuel
                        </h3>
                        <div className="mt-2 flex items-center gap-3">
                          <UserAvatar
                            user={{
                              pseudo: players[current]?.name,
                              photoProfil: players[current]?.photo,
                            }}
                            size="sm"
                          ...
                          <div>
                            <div className="text-sm font-bold text-white">
                              {players[current]?.name ?? "En attente"}
                            </div>
                            <div className="text-xs text-emerald-50/80">
                              {pendingStart
                                ? `Départ dans ${countdown ?? 0}s`
                                : started
                                  ? `C'est au tour de ${players[current]?.name ?? "..."} 🎲`
                      ...
                      <div className="mb-4 rounded-[22px] border border-white/10 bg-white/12 p-3">
                        ...
                        <div className="mt-3 text-2xl font-black text-[#d9ffea]">
                          {raceTime}s
                        </div>
                      </div>
                        ...
                        <div className="mt-2 flex items-center gap-3">
                          ...
                          <div>
                            <Button
                              ...
                              size="sm"
                              className="mb-2 rounded-full bg-gradient-to-r from-emerald-400 to-lime-400 text-slate-900 shadow-[0_12px_22px_rgba(16,185,129,0.28)]"
                            >
                              Lancer le dé
                            </Button>
                            <div className="text-xs text-emerald-50/80">
      ...
      {pendingStart && countdown !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4 backdrop-blur-[2px]">
          <div className="countdown-glow w-full max-w-md rounded-[32px] border border-emerald-200 bg-gradient-to-br from-white via-emerald-50 to-lime-50 p-6 text-center shadow-[0_28px_70px_rgba(16,185,129,0.28)]">
            ...
            <h2 className="text-2xl font-extrabold uppercase tracking-[0.25em] text-slate-800">
              Start
            </h2>
            <p className="mt-2 text-sm text-[#64748b]">
              L’équipe est prête. La partie commence dans {countdown} seconde
              {countdown > 1 ? "s" : ""}.
            </p>
            <div className="countdown-number my-6 text-6xl font-black text-[#16a34a] drop-shadow-[0_12px_18px_rgba(16,185,129,0.25)]">
              {countdown}
            </div>
          </div>
        </div>
