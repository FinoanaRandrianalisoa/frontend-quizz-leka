...
export default function PenaltyMatchPage({
  ...
  const loadPendingChallenge = async () => {
    ...
    try {
      ...
      const activeMatch = (res.mesMatchsPenalty ?? []).find(
  ...
  const pollMatch = async () => {
    ...
    try {
      const res = await api.mesMatchsPenalty()
      const currentMatch = (res.mesMatchsPenalty ?? []).find(
    ...
    try {
      ...
      const loser =
        match.scoreHote > match.scoreInvite
          ...
          : match.joueurHotePseudo
      await api.publier(
        `⚽ ${winner} a gagné un duel de tir au but contre ${loser} ! Score final : ${match.scoreHote} - ${match.scoreInvite}`,
      )
      setShowVictoryBadge(false)
    } catch (error: any) {
  ...
  async function playShot(direction: Direction) {
    ...
    try {
      ...
      if (
        ...
        try {
          const res = await api.mesMatchsPenalty()
          const fallback = (res.mesMatchsPenalty ?? []).find(
            (item) => item.statut === "en_cours",
          )
          if (fallback) {
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm p-4 md:p-6 shadow-xl">
              ...
              <PenaltyField3D
                ...
                goalkeeperPosition={
                  currentDirection as "gauche" | "centre" | "droite" ||
                  undefined
                }
              />
