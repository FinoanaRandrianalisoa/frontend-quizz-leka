...
export default function SquidGamePage({
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("notifications:refresh", refreshChallenges)
  }, [user?.id])
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(
      `${wsBase}/ws/rps/${match.id}/?token=${encodeURIComponent(token)}`,
    )
  ...
  const statusText = useMemo(() => {
    if (challengeId && selected)
      return `Défi reçu de ${selected.pseudo} — accepte pour démarrer le duel.`

    if (!match) return "Défiez un autre joueur en ligne pour commencer."

    if (match.statut === "termine") {
      const winner =
        String(match.joueurHoteId) === String(match.joueurHoteId) &&
        match.scoreHote >= match.scoreCible
          ? match.joueurHotePseudo
          : match.joueurInvitePseudo
  ...
  async function acceptChallenge() {
    ...
    try {
      ...
      const validChallenge = pending.mesDefisRps?.find(
  ...
  async function playMove(move: Move) {
    ...
    try {
      ...
      if (
        ...
        msg.includes("Partie Rock Paper Scissors")
      ) {
        try {
          const res = await api.mesMatchsRps()

          const fallback = (res.mesMatchsRps ?? []).find(
      ...
      <div className="mb-6 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-[#64748b]">
            Pierre, Papier, Ciseaux en ligne multijoueur.
          </p>
        </div>
            ...
            {match ? (
              <div className="space-y-5">
                <div className="grid grid-cols-3 gap-3">
                  {MOVES.map((move) => (
                    <button
                      ...
                      disabled={
                        loading || roundLocked || match.statut === "termine"
                      }
                      className={`rounded-2xl border p-4 text-center transition ${
                        currentMove === move
                          ? "border-[#16a34a] bg-[#f0fdf4]"
                          : "border-[#dfeae0] bg-white hover:border-[#16a34a]"
                      }`}
                    >
                ...
                <div className="rounded-2xl border border-dashed border-[#d9e7dd] bg-[#f9fdf9] p-4 text-sm text-[#64748b]">
                  {meIsHost
                    ? "Tu es l'hôte du défi."
                    : "Tu es l'invité du défi."}
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="rounded-2xl border border-dashed border-[#d9e7dd] bg-[#f9fdf9] p-6 text-center text-[#64748b]">
                  Aucune partie active pour le moment. Choisissez un joueur pour
                  envoyer un défi.
                </div>

                {activeMatches.length > 0 && (
                  <div className="rounded-2xl border border-[#edf2ee] bg-white p-4">
                    <p className="mb-3 text-sm font-semibold text-[#1f2a1f]">
                      Parties lancées
                    </p>
                    <div className="space-y-2">
                      {activeMatches.map((active) => {
                        const opponent =
                          String(active.joueurHoteId) === String(user?.id)
                          ...
                          >
                            <div>
                              ...
                              <div className="text-xs text-[#64748b]">
                                Manche {active.round} · {active.scoreHote} -{" "}
                                {active.scoreInvite}
                              </div>
                            </div>
                ...
                {spectatorMatches.length > 0 && (
                  <div className="rounded-2xl border border-[#e7d8a9] bg-[#fff9eb] p-4">
                    <p className="mb-3 text-sm font-semibold text-[#8a5b00]">
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <div className="text-xs text-[#64748b]">
                                Manche {spectatorMatch.round} ·{" "}
                                {spectatorMatch.scoreHote} -{" "}
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <div className="mb-4 rounded-xl border border-[#edf2ee] bg-[#f8fafc] p-3">
              <label className="mb-2 block text-xs font-medium uppercase tracking-[0.2em] text-[#64748b]">
                Score cible
              </label>
              <input
                type="number"
            ...
            <div className="space-y-3">
              {players.length === 0 ? (
                ...
                players.map((player) => (
                  <div
                    key={player.id}
                    className="flex items-center justify-between gap-3 rounded-xl border border-[#edf2ee] p-3"
                  >
                    <div>
                      <p className="font-medium">{player.pseudo}</p>
                      <p className="text-xs text-[#64748b]">
                        Disponible pour un duel
                      </p>
                    </div>
                    <Button
                      size="sm"
                      onClick={() => void challengePlayer(player)}
                      disabled={loading}
                    >
                      Défi
                    </Button>
            ...
            {challengeId && !match && (
              <div className="mt-6 rounded-2xl border border-[#e7d8a9] bg-[#fff9eb] p-4 text-sm text-[#8a5b00]">
                <p className="font-medium">
                  Défi envoyé à {selected?.pseudo ?? "ce joueur"}.
                </p>
