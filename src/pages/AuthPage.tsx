...
export default function AuthPage({
  ...
  useEffect(() => {
    ...
    if (
      current === "" ||
      (autoBaseRef.current && current.startsWith(autoBaseRef.current))
    ) {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        if (r.pseudoInfo.disponible) {
          ...
          if (
            !touchedRef.current &&
            r.pseudoInfo.suggestion &&
            r.pseudoInfo.suggestion !== val
          )
            setName(r.pseudoInfo.suggestion)
        }
      } catch {
        setUsernameStatus("idle")
      }
  ...
  useEffect(() => {
    if (mode === "register") {
      api
        ...
        .then((data) =>
          setVilles(
            data.villes.map((ville) => ({ id: ville.id, nom: ville.nom })),
          ),
        )
        .catch(() => setVilles([]))
    }
  ...
  const handleLocalImage = (
    ...
    if (file.size > MAX_LOCAL_IMAGE_BYTES) {
      setError(
        "L’image est trop lourde. Sélectionnez une photo de moins de 2 Mo.",
      )
      e.target.value = ""
      return
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "register") {
      if (usernameStatus === "taken") {
        setError(
          "Ce nom d'utilisateur est déjà pris. Choisissez-en un autre (ou cliquez sur la suggestion).",
        )
        return
      }
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-10">
            ...
            <span className="text-3xl font-extrabold">
              Quizz <span className="text-[#86efac]">Leka</span>
            </span>
          </div>
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-8 bg-[#F9F9F9]">
        <div className="w-full max-w-md">
          <div className="flex items-center gap-2 mb-8 lg:hidden">
            ...
            <span className="text-2xl font-extrabold text-[#2D3142]">
              Quizz <span className="text-[#16a34a]">Leka</span>
            </span>
          </div>
              ...
              {mode === "forgot" ? (
                <>
                  ...
                  <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    {error && (
                      <p className="text-sm text-[#D62828] bg-[#D62828]/10 rounded-lg px-3 py-2">
                        {error}
                      </p>
                    )}
                    <Input
                ...
                <>
                  ...
                  <p className="text-sm text-[#A0A0A0] mb-6">
                    Rejoignez la communauté Quizz Leka.
                  </p>
                  <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    {error && (
                      <p className="text-sm text-[#D62828] bg-[#D62828]/10 rounded-lg px-3 py-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          ...
                          <div className="sm:col-span-2 flex flex-col gap-1.5">
                            <label
                              ...
                              className="text-sm font-medium text-[#2D3142]"
                            >
                              Nom d'utilisateur *
                            </label>
                            <div className="relative flex items-center">
                              ...
                              <input
                                ...
                                className={`w-full rounded-lg border text-sm bg-white text-[#2D3142] placeholder:text-[#A0A0A0] pl-9 pr-9 py-2 transition-colors focus:outline-none focus:ring-2 focus:ring-[#16a34a] ${
                                  usernameStatus === "taken"
                                    ...
                                    : usernameStatus === "ok"
                                      ? "border-[#06A77D] focus:border-[#06A77D]"
                                      : "border-[#D9D9D9]"
                                }`}
                                required
                              />
                              <span className="absolute right-3">
                                {usernameStatus === "checking" ? (
                                  <Loader2
                                    size={16}
                                    className="text-[#A0A0A0] animate-spin"
                                  />
                                ) : usernameStatus === "ok" ? (
                            ...
                            {usernameStatus === "ok" && (
                              <p className="text-xs text-[#06A77D]">
                                Disponible ✓
                              </p>
                            )}
                            {usernameStatus === "taken" && (
                              <p className="text-xs text-[#D62828]">
                                ...
                                {usernameSuggestion &&
                                  usernameSuggestion !== name.trim() && (
                                    <>
                                      {" "}
                                      <button
                                        type="button"
                                        className="text-[#16a34a] font-medium underline"
                                        onClick={() =>
                                          setName(usernameSuggestion)
                                        }
                                      >
                                        Utiliser {usernameSuggestion}
                                      </button>
                                    </>
                                  )}
                              </p>
                          ...
                          <div className="flex flex-col gap-1.5">
                            ...
                            <div className="relative flex items-center">
                              ...
                              <select
                                value={villeOrigine}
                                onChange={(e) =>
                                  setVilleOrigine(e.target.value)
                                }
                                className="w-full rounded-lg border border-[#D9D9D9] text-sm bg-white text-[#2D3142] pl-9 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#16a34a] focus:border-[#16a34a]"
                                required
                              >
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            <input
                              type="file"
                              accept="image/*"
                              onChange={(e) => handleLocalImage(e, "profil")}
                            ...
                            {photoProfil && (
                              <img
                                src={photoProfil}
                                alt="Photo profil"
                                className="h-16 w-16 rounded-full object-cover border border-[#D9D9D9]"
                              />
                            )}
                          </div>
                          <div className="flex flex-col gap-2">
                            ...
                            {photoCouverture && (
                              <img
                                ...
                                className="h-20 w-full rounded-xl object-cover border border-[#D9D9D9]"
                              />
                            )}
                          </div>
                        </div>
