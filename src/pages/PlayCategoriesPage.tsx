...
export default function PlayCategoriesPage({
  onNavigate,
}: {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-8">
      ...
      <p className="text-sm text-[#64748b] mb-6">
        Sélectionnez le type de jeu que vous souhaitez lancer.
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {categories.map((c) => (
          ...
          >
            <div className="w-12 h-12 rounded-xl bg-[#f1faf5] flex items-center justify-center text-[#16a34a]">
              <c.icon size={20} />
            </div>
            <div>
              <h3 className="text-lg font-bold">{c.label}</h3>
              <p className="text-sm text-[#64748b]">
                Jouez des défis en direct pour {c.label}.
              </p>
            </div>
          </button>
        ))}
