...
export default function LobbyPage({
  ...
  const list = (matches.data ?? []).filter((m) => {
    ...
    const okSearch =
      !q || m.joueurHote.pseudo.toLowerCase().includes(q) || cat.includes(q)
  ...
  const createMatch = async (id?: string) => {
    ...
    const target =
      Number.isFinite(scoreCible) && scoreCible > 0 ? scoreCible : 8
    if (!chosen) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 pb-20 md:pb-8">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-[#A0A0A0]">
            {matches.data?.length ?? 0} parties en attente
          </p>
        </div>
        <div className="sm:ml-auto flex flex-col sm:flex-row gap-2">
          ...
          <input
            ...
            value={scoreCible}
            onChange={(e) =>
              setScoreCible(Math.max(1, Number(e.target.value || 1)))
            }
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm ${
                category === cat
                  ...
                  : "bg-white border border-[#D9D9D9] text-[#2D3142]"
              }`}
            >
              {cat}
            </button>
      ...
      {matches.error && (
        <p className="text-sm text-[#D62828] mb-4">{matches.error}</p>
      )}
              ...
              <CardContent className="p-4 flex flex-col gap-3">
                ...
                <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#F5F5F5]">
                  <div
                    ...
                    style={{ background: themeColor(m.theme.nom) + "20" }}
                  >
                    <Globe
                      size={12}
                      style={{ color: themeColor(m.theme.nom) }}
                    />
                  </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-[#A0A0A0] flex-1">
                    En attente d'adversaire
                  </span>
                  <Button
                    size="sm"
                    loading={joining === m.id}
