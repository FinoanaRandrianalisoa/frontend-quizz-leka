...
const podiumColors: Record<number, {
  bg: string
  text: string
  icon: typeof Crown
}> = {
    ...
    <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 pb-20 md:pb-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-[#A0A0A0]">
          Les meilleurs joueurs Quizz Leka selon les données du serveur
        </p>
      </div>
                  ...
                  return (
                    ...
                    >
                      <Avatar size={actualRank === 1 ? "xl" : "lg"}>
                        <AvatarFallback>
                          {initial(entry.utilisateur.pseudo)}
                        </AvatarFallback>
                      </Avatar>
                      <p className="text-xs font-semibold text-[#2D3142] text-center truncate w-full px-1">
                      ...
                      <div
                        className={`w-full ${heights[gridIdx]} rounded-t-xl border-2 ${bg} flex flex-col items-center justify-center gap-1`}
                      >
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {entry.utilisateur.role === "ADMIN" && (
                              <Badge variant="category">Admin</Badge>
                            )}
                          </div>
                          <p className="text-[10px] text-[#A0A0A0]">
                            {entry.parties} parties · {entry.victoires}V ·{" "}
                            {entry.defaites}D
                          </p>
                        </div>
                        <div className="hidden sm:flex flex-col items-end gap-1 w-20">
                          <span className="text-xs text-[#A0A0A0]">
                            WR {entry.tauxReussite}%
                          </span>
                          <Progress
                            value={entry.tauxReussite}
                            className="h-1"
                            color={
                              entry.tauxReussite >= 80
                                ? "#06A77D"
                                : entry.tauxReussite >= 60
                                  ? "#FF6B35"
                                  : "#A0A0A0"
                            }
                          />
                        </div>
                        <span className="text-sm font-bold text-[#FF6B35] tabular-nums shrink-0">
                          {entry.victoires}V
                        </span>
                      </div>
                    )
