...
export default function CommunityPage({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  const [chatMessages, setChatMessages] = useState<Array<{ id: string; contenu: string; utilisateur: { pseudo: string }; creeLe: string }>>([]);
  ...
  useEffect(() => {
    ...
    void api.messagesSalon(activeCity, 12)
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 pb-16 md:px-6">
      <div className="mb-6 rounded-3xl bg-gradient-to-r from-[#16a34a] to-[#22c55e] p-5 text-white shadow-sm">
        <p className="text-[10px] uppercase tracking-[0.22em] text-white/70">Communauté</p>
        ...
        <CardContent className="p-4 md:p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {(villes.data ?? []).map((ville) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${activeCity === ville.slug ? "bg-[#16a34a] text-white" : "bg-[#f3faf4] text-[#1f2a1f]"}`}
                >
                  {ville.nom}
                </button>
              ))}
          ...
          <div className="rounded-3xl border border-[#dfeae0] bg-[#f9fdf9] p-3 md:p-4">
            <div className="mb-3 flex items-center gap-2 font-semibold text-[#1f2a1f]">
              ...
              Salon live : {villes.data?.find((ville) => ville.slug === activeCity)?.nom ?? "Ville"}
            </div>
            ...
            <div className="space-y-3">
              {chatMessages.length === 0 ? (
                ...
                chatMessages.map((entry) => (
                  <div key={entry.id} className="rounded-2xl border border-[#edf6ef] bg-white p-3 shadow-sm">
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-sm font-semibold text-[#1f2a1f]"><UserName user={entry.utilisateur} /></span>
                      <span className="text-[10px] text-[#64748b]">{relativeTime(entry.creeLe)}</span>
                    </div>
                    <p className="mt-1 text-sm leading-relaxed text-[#334155]">{entry.contenu}</p>
                  </div>
                ))
              )}
            </div>
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              />
              <Button size="sm" className="gap-2" onClick={() => void handleSendCityMessage()}>
