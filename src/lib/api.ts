...
export const api = {
  ...
  themes() {
    return gql<{ themes: Theme[] }>(
      `{ themes { id nom slug description icone nombreQuestions } }`,
    )
  },
  villes() {
  ...
  marquerMessagesLus(amiId: number) {
    return gql<{ marquerMessagesLus: boolean }>(
      `mutation($amiId: Int!) { marquerMessagesLus(amiId: $amiId) }`,
      { amiId },
    )
  },
  ...
  mesParties() {
    return gql<{ mesParties: Match[] }>(`{ mesParties { ${MATCH_DETAIL} } }`)
  },
  ...
  annulerPartie(matchId: number) {
    return gql<{ annulerPartie: boolean }>(
      `mutation($id: Int!) { annulerPartie(matchId: $id) }`,
      { id: matchId },
    )
  },
  ...
  mesDefisPenalty() {
    return gql<{
      mesDefisPenalty: Array<{
        id: string
        hoteId: string
        hotePseudo: string
        inviteId: string
        invitePseudo?: string | null
        scoreCible: number
        statut: string
      }>
    }>(
