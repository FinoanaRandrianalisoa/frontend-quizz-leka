...
export function nameParts(user?: { pseudo?: string | null; firstName?: string | null; villeOrigine?: string | null } | null): {
  prenom: string;
  ville: string;
} {
...
export function displayName(user?: { pseudo?: string | null; firstName?: string | null; villeOrigine?: string | null } | null): string {
  const { prenom, ville } = nameParts(user);
  return ville ? `${prenom} de ${ville}` : prenom;
}

const THEME_COLORS = ["#FF6B35", "#004E89", "#06A77D", "#F59E0B", "#9B59B6", "#E74C3C", "#3498DB", "#E91E63"];

export function themeColor(name: string, index = 0): string {
  let h = 0;
  for (let i = 0; i < name.length; i++) h = (h + name.charCodeAt(i) * (i + 1)) % THEME_COLORS.length;
