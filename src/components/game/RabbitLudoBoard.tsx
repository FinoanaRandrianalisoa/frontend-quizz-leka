  ...
  return (
    <div className="relative h-full w-full rounded-[14px] bg-gradient-to-br from-white to-slate-100">
      {slots.map((slot) => (
        <span
          key={slot}
          className={`absolute h-2.5 w-2.5 rounded-full bg-slate-800 ${pos[slot]}`}
        />
      ))}
    </div>
  )
...
export default function RabbitLudoBoard({
  ...
  const pathIndexByKey = new Map(
    PATH_CELLS.map((cell, i) => [`${cell.r}-${cell.c}`, i + 1]),
  )
      ...
      <div className="ludo-board relative aspect-square w-full">
        ...
        <div className="ludo-board-face absolute inset-0 overflow-hidden rounded-[28px] border-[10px] border-[#7a3f16] bg-[#1f6b3a] shadow-[0_30px_0_#5b2c0c,0_48px_40px_rgba(0,0,0,0.35)]">
              ...
              return (
                <div
                  ...
                  className={[
                    ...
                    cage
                      ? "bg-gradient-to-br from-[#fff7d6] to-[#f1d48a]"
                      : "bg-transparent",
                    isCenter
                  ...
                  style={
                    trail && cage
                      ? { boxShadow: `inset 0 0 0 2px ${trail.color}` }
                      : undefined
                  }
                >
                  {cage ? (
                    <span className="absolute left-0.5 top-0.5 text-[8px] font-black text-[#7a4e12]/80">
                      {cage}
                    </span>
                  ) : null}
                  {isCenter && r === 4 && c === 4 ? (
                    <div className="absolute inset-0 flex items-center justify-center text-lg drop-shadow">
                      🏁
                    </div>
                  ) : null}
                </div>
          ...
          {players.map((player, index) => {
            ...
            const offset = players
              .filter((p) => p.pos === player.pos)
              .findIndex((p) => p.id === player.id)
            return (
              <div
                key={player.id}
                className={`ludo-token absolute z-20 flex items-end justify-center ${
                  hoppingId === player.id ? "ludo-token-hop" : ""
                }`}
                style={{
              ...
              >
                <div className="relative flex h-[78%] w-[70%] flex-col items-center">
                  {currentPlayerId === player.id && (
                    <span className="absolute -top-3 text-[10px] text-amber-200">
                      ▼
                    </span>
                  )}
                  <div
...
export function VictoryOverlay({
  ...
  const bits = Array.from({ length: 42 }, (_, i) => ({
    ...
    left: `${(i * 17) % 100}%`,
    delay: `${(i % 8) * 0.08}s`,
    color: ["#ef4444", "#f59e0b", "#22c55e", "#3b82f6", "#e879f9", "#fde68a"][
      i % 6
    ],
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-black/55 p-4">
      ...
      <div className="relative w-full max-w-lg rounded-[32px] border border-amber-200 bg-gradient-to-br from-white via-amber-50 to-emerald-50 p-6 text-center shadow-[0_30px_80px_rgba(250,204,21,0.28)]">
        ...
        <h2 className="mt-2 text-2xl font-extrabold tracking-wide text-slate-800">
          FÉLICITATIONS
        </h2>
        <p className="mt-2 text-lg font-semibold text-emerald-800">
          {name} atteint la cage finale !
        </p>
        <p className="mt-1 text-sm text-slate-500">
          {rematchRequestedBy
            ? isWinner
              ? "Une demande de revanche a été reçue. Acceptez ou refusez."
              : "En attente de la réponse du gagnant..."
            : isWinner
              ? "Publiez votre victoire ou relancez une revanche."
              : "Proposez une revanche ou quittez la partie."}
        </p>
            ...
            <>
              {isWinner && onAcceptRematch && (
                <button
                  ...
                  className="rounded-full bg-gradient-to-r from-emerald-500 to-lime-400 px-4 py-2.5 text-sm font-bold text-slate-900"
                >
              ...
              {isWinner && onRejectRematch && (
                <button
                  ...
                  className="rounded-full border border-red-200 bg-red-50 px-4 py-2.5 text-sm font-semibold text-red-700"
                >
                  Refuser
                </button>
              )}
