...
export default function TopNav({ currentPage, onNavigate }: TopNavProps) {
  ...
  const loadInviteData = async () => {
    try {
      const [friendsResponse, requestsResponse, sentResponse, usersResponse] =
        await Promise.all([
          api.mesAmis(),
  ...
  useEffect(() => {
    ...
    const loadNotifications = async () => {
      try {
        ...
        if (!active) return
        const unread = (response.mesNotifications ?? []).filter(
          (item) => !item.lu,
        ).length
    ...
    const scheduleReconnect = () => {
      ...
      const delay = Math.min(
        30000,
        Math.pow(2, Math.min(6, reconnectAttempts)) * 1000,
      )
      // debug
      ...
      socket.onmessage = (event) => {
        try {
          ...
          window.dispatchEvent(
            new CustomEvent("notifications:refresh", { detail: payload }),
          )
        } catch (e) {
          setNotificationCount((current) => current + 1)
    ...
    window.addEventListener(
      ...
      onNotificationsRefresh as EventListener,
    )
  ...
  const candidateUsers = allUsers
    .filter((item) => {
      ...
      return (
        ...
        (item.villeOrigine ?? "").toLowerCase().includes(term)
      )
    })
    .slice(0, 50)
  ...
  useEffect(() => {
    return () => {
      if (inviteErrorTimer.current)
        window.clearTimeout(inviteErrorTimer.current)
    }
  }, [])
          ...
          {navLinks.map(({ id, label, icon: Icon }) => (
            ...
            >
              ...
              {id === "messages" && unreadMessagesCount > 0 && (
                <span className="ml-2 inline-flex items-center justify-center rounded-full bg-[#ef4444] text-white text-[10px] px-2 py-0.5 font-semibold">
                  {unreadMessagesCount}
                </span>
              )}
            </button>
        ...
        <div className="relative flex-1 min-w-0 mx-1 sm:mx-3">
          ...
          {globalSearchFocused && (
            <div className="absolute left-0 right-0 top-full mt-2 max-h-[70vh] overflow-y-auto rounded-2xl border border-[#d9e7dd] bg-white shadow-xl z-50 p-2">
              {inviteError && (
                <p className="px-2 pb-2 text-xs font-medium text-[#D62828]">
                  {inviteError}
                </p>
              )}
              <p className="px-2 pt-1 pb-2 text-[11px] font-bold uppercase tracking-[0.12em] text-[#64748b]">
                Joueurs
              </p>
                ...
                <div className="space-y-1">
                  {globalResults.map((item) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        className="flex items-center gap-2 rounded-xl px-2 py-1.5 hover:bg-[#f9fdf9]"
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="truncate text-[10px] text-[#64748b]">
                            {item.villeOrigine || "Ville non renseignée"}
                          </p>
                        </div>
                        {sent ? (
                          ...
                          <Button
                            size="sm"
                            variant="secondary"
                            onClick={() => void sendInvite(item.pseudo)}
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <div className="relative">
            ...
            >
              ...
              <span className="hidden sm:block text-sm font-semibold text-[#1f2a1f]">
                <UserName user={user} />
              </span>
              <ChevronDown
                size={14}
                className="text-[#64748b] hidden sm:block"
              />
            </button>
            {userMenuOpen && (
              <div className="absolute right-0 top-full mt-1 w-52 bg-white border border-[#d9e7dd] rounded-xl shadow-lg overflow-hidden z-50">
                <div className="p-3 border-b border-[#d9e7dd]">
                  <p className="font-semibold text-sm text-[#1f2a1f]">
                    <UserName user={user} />
                  </p>
                ...
                {[
                  ...
                  ...(isAdmin
                    ? [{ icon: Shield, label: "Utilisateurs", page: "admin" }]
                    : []),
          ...
          <div className="absolute left-3 right-3 sm:left-auto sm:right-6 sm:w-[400px] top-full z-50 mt-0 sm:mt-2 max-h-[80vh] overflow-y-auto rounded-2xl border border-[#d9e7dd] bg-white shadow-xl">
            <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-[#edf6ef]">
              ...
              <button
                onClick={closeInvite}
                aria-label="Fermer"
                className="text-[#A0A0A0] hover:text-[#1f2a1f]"
              >
                <X size={16} />
              </button>
            </div>
            ...
            {inviteError && (
              <p className="px-4 py-2 text-xs font-medium text-[#D62828] bg-[#D62828]/5">
                {inviteError}
              </p>
            )}
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {friendRequests.map((request) => (
                      <div
                        key={request.id}
                        className="flex flex-wrap items-center gap-2 rounded-xl border border-[#edf6ef] bg-[#f9fdf9] p-2"
                      >
                        <UserAvatar user={request.demandeur} />
                        <div className="min-w-0 flex-1 basis-32">
                          <p className="truncate text-sm font-semibold text-[#1f2a1f]">
                            <UserName user={request.demandeur} />
                          </p>
                          <p className="truncate text-[10px] text-[#64748b]">
                            veut devenir votre ami ·{" "}
                            {relativeTime(request.creeLe)}
                          </p>
                        </div>
                        <div className="flex items-center gap-1.5 ml-auto">
                          <Button
                            ...
                            variant="success"
                            onClick={() =>
                              void answerInvite(Number(request.id), true)
                            }
                          >
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            className="border-[#f3c2c2] text-[#D62828] hover:bg-[#D62828]/5"
                            onClick={() =>
                              void answerInvite(Number(request.id), false)
                            }
                          >
                            <X size={14} /> Refuser
                          </Button>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {sentRequests.map((request) => (
                      <div
                        key={request.id}
                        className="flex items-center gap-2 rounded-xl border border-[#edf6ef] bg-[#f9fdf9] p-2"
                      >
                        <UserAvatar user={request.receveur} />
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-sm font-semibold text-[#1f2a1f]">
                            <UserName user={request.receveur} />
                          </p>
                          <p className="truncate text-[10px] text-[#64748b]">
                            En attente · {relativeTime(request.creeLe)}
                          </p>
                        </div>
              ...
              <div>
                <div className="flex items-center justify-between gap-2 mb-2">
                  ...
                  <span className="text-[10px] font-semibold text-[#64748b]">
                    {candidateUsers.length} à ajouter
                  </span>
                </div>
                  ...
                  <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
                    {candidateUsers.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className="flex flex-wrap items-center gap-2 rounded-xl border border-[#edf6ef] p-2"
                        >
                          <UserAvatar user={item} online={item.enLigne} />
                          <div className="min-w-0 flex-1 basis-32">
                            <p className="truncate text-sm font-semibold text-[#1f2a1f]">
                          ...
                          {sent ? (
                            <span className="shrink-0 ml-auto text-[11px] font-semibold text-[#15803d]">
                              Envoyée {relativeTime(sent.creeLe)}
                            </span>
                          ) : (
                            <Button
                ...
                className={`relative flex h-10 w-10 items-center justify-center rounded-xl border transition-all
                  ${
                    currentPage === id
                      ...
                      : "bg-white text-[#1f2a1f] border-[#d9e7dd]"
                  }`}
              >
                <Icon size={18} />
                {id === "messages" && unreadMessagesCount > 0 && (
            ...
            <button
              ...
              className={`relative flex h-10 w-10 items-center justify-center rounded-xl border transition-all ${
                inviteMenuOpen
                  ...
                  : "border-[#d9e7dd] bg-white text-[#1f2a1f]"
              }`}
            >
              <UserPlus size={18} />
              {friendRequests.length > 0 && (
