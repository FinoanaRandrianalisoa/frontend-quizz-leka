...
const variantStyles: Record<ButtonVariant, string> = {
  default:
    "bg-[#16a34a] hover:bg-[#15803d] active:bg-[#166534] text-white shadow-sm",
  secondary:
    "bg-[#f3faf4] hover:bg-[#eaf7ee] text-[#0f766e] border border-[#d9e7dd]",
  outline:
    "bg-transparent hover:bg-[#f3faf4] text-[#1f2a1f] border border-[#d9e7dd]",
...
export function Card({ children, className = "", ...props }: CardProps) {
  return (
    <div
      className={`bg-white border border-[#D9D9D9] rounded-xl shadow-sm ${className}`}
      {...props}
    >
      {children}
    </div>
...
export function CardTitle({ children, className = "", ...props }: CardProps) {
  return (
    <h3 className={`text-lg font-bold text-[#2D3142] ${className}`} {...props}>
      {children}
    </h3>
  )
}
...
export function Badge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${badgeStyles[variant]} ${className}`}
      {...props}
    >
      {children}
    </span>
...
export function Avatar({
  ...
  return (
    <div className={`relative inline-flex shrink-0 ${className}`} {...props}>
      <div
        className={`${avatarSize[size]} rounded-full overflow-hidden bg-[#16a34a]/10 flex items-center justify-center font-semibold text-[#15803d]`}
      >
        {children}
      </div>
      {online !== undefined && (
...
export function Input({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      {label && (
        <label htmlFor={inputId} className="text-sm font-medium text-[#2D3142]">
          {label}
        </label>
      )}
      <div className="relative flex items-center">
        {leftIcon && (
          <span className="absolute left-3 text-[#A0A0A0]">{leftIcon}</span>
        )}
        <input
          id={inputId}
          className={`
        ...
        />
        {rightIcon && (
          <span className="absolute right-3 text-[#A0A0A0]">{rightIcon}</span>
        )}
      </div>
...
export function Progress({
  ...
  return (
    <div
      className={`w-full bg-[#F5F5F5] rounded-full overflow-hidden h-2 ${className}`}
    >
      <div
...
export function TabsList({
  ...
  return (
    <div className={`flex gap-1 bg-[#F5F5F5] p-1 rounded-lg ${className}`}>
      {children}
    </div>
  )
}
...
export function DialogTitle({
  ...
  return (
    <h2 className={`text-xl font-bold text-[#2D3142] ${className}`}>
      {children}
    </h2>
  )
}
...
export function DialogFooter({
  ...
  return (
    <div className={`p-6 pt-0 flex gap-2 justify-end ${className}`}>
      {children}
    </div>
  )
}
...
export function Sheet({ open, onClose, children, side = "right" }: SheetProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      />
      <div
        className={`absolute bg-white shadow-2xl flex flex-col animate-fade-in-scale ${slideClass}`}
      >
        {children}
      </div>
    </div>
...
export function SheetHeader({
  ...
  return (
    <div
      className={`p-4 border-b border-[#D9D9D9] flex items-center justify-between ${className}`}
    >
      {children}
    </div>
  )
...
export function Select({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      {label && (
        <label className="text-sm font-medium text-[#2D3142]">{label}</label>
      )}
      <select
    ...
    <ToastContext.Provider value={{ toast }}>
      ...
      <div className="fixed bottom-20 md:bottom-6 right-4 md:right-6 z-[100] flex flex-col gap-2 max-w-xs w-full">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`${colors[t.type]} px-4 py-3 rounded-lg shadow-lg text-sm font-medium animate-fade-in-up flex items-center gap-2`}
          >
            {t.message}
          </div>
        ))}
