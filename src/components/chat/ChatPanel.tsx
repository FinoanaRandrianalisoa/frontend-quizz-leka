...
export default function ChatPanel({
    ...
    if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
      try {
        socketRef.current.send(
          JSON.stringify({
            type: "message",
            envoyeur: user?.pseudo ?? "spectateur",
            message: texte,
          }),
        )
      } catch {
        ...
        setMessages((m) => [
          ...m,
          {
            ...
            from: "me",
            text: texte,
            time: new Date().toLocaleTimeString("fr-MG", {
              hour: "2-digit",
              minute: "2-digit",
            }),
          },
  ...
  return (
    <div
      className={`flex flex-col bg-white border-l border-[#D9D9D9] ${className}`}
    >
      {/* Header */}
            ...
            if (msg.from === "system") {
              return (
                <div key={msg.id} className="text-center">
                  <span className="text-[10px] text-[#A0A0A0] bg-[#F5F5F5] px-2 py-0.5 rounded-full">
                    {msg.text}
                  </span>
                </div>
              )
